...
function handle_mousemove(e) {
    let pd = new PointerEvent('pointdown', {
        button: 2
    })
    let pu = new PointerEvent('pointup', {
        button: 2
    })
    document.getElementsByTagName('canvas')[0].dispatchEvent(pd)
    ...
    document.getElementsByTagName('canvas')[0].dispatchEvent(pu)
    e.preventDefault();
}

function handle_mousedown(f) {
    let e
    if (!f.clientX) {
        e = new PointerEvent(f.type, {
            clientX : this.sc.mousex,
            button: 2,
            clientY: this.sc.mousey
        })
    } else {
        e = f
    }
...
function sendRightClick() {
    ...
    const e2 = new MouseEvent('pointerup', {
        button: 2
    })
    document.getElementsByTagName('canvas')[0].dispatchEvent(e)
    document.getElementsByTagName('canvas')[0].dispatchEvent(e2)
}

function handle_touchmove() {
    e.preventDefault();
}
