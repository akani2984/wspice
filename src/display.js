...
SpiceDisplayConn.prototype.process_channel_message = function(msg)
{
    ...
    if (msg.type == Constants.SPICE_MSG_DISPLAY_SURFACE_CREATE)
    {
        ...
        if (m.surface.flags & Constants.SPICE_SURFACE_FLAGS_PRIMARY)
        {
            ...
            //document.getElementById(this.parent.screen_id).style.height = '100vh';
            this.hook_events();
        }
        return true;
    }
...
SpiceDisplayConn.prototype.hook_events = function()
{
    if (this.primary_surface !== undefined)
    {
        ...
        canvas.addEventListener('pointermove', Inputs.handle_mousemove);
        canvas.addEventListener('pointerdown', Inputs.handle_mousedown);
        canvas.addEventListener('contextmenu', Inputs.handle_contextmenu);
        canvas.addEventListener('pointerup', Inputs.handle_mouseup);
        canvas.addEventListener('keydown', Inputs.handle_keydown);
        canvas.addEventListener('keyup', Inputs.handle_keyup);
        canvas.addEventListener('pointerout', handle_mouseout);
        canvas.addEventListener('pointerover', handle_mouseover);
        canvas.addEventListener('wheel', Inputs.handle_mousewheel);
        document.getElementById('rightClickBtn').addEventListener('click', Inputs.sendRightClick)
        canvas.focus();
...
SpiceDisplayConn.prototype.unhook_events = function()
{
    ...
    {
        var canvas = this.surfaces[this.primary_surface].canvas;
        canvas.removeEventListener('pointermove', Inputs.handle_mousemove);
        canvas.removeEventListener('pointerdown', Inputs.handle_mousedown);
        canvas.removeEventListener('contextmenu', Inputs.handle_contextmenu);
        canvas.removeEventListener('pointerup', Inputs.handle_mouseup);
        canvas.removeEventListener('keydown', Inputs.handle_keydown);
        canvas.removeEventListener('keyup', Inputs.handle_keyup);
        canvas.removeEventListener('pointerout', handle_mouseout);
        canvas.removeEventListener('pointerover', handle_mouseover);
        canvas.removeEventListener('wheel', Inputs.handle_mousewheel);
        canvas.removeEventListener('focus', this.focusListener);
        //canvas.removeEventListener('touchmove', Inputs.handle_touchmove);
        document.getElementById('rightClickBtn').removeEventListener('click', Inputs.sendRightClick)
    }
}
